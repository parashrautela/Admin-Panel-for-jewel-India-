...
import { fetchWholesalers, fetchStatusCounts, type WholesalerStatus, type WholesalerRecord } from '../../../lib/adminApi';

export function AdminDashboard() {
  const [selectedFilter, setSelectedFilter] = useState<WholesalerStatus | 'all'>('all');
  const [searchQuery, setSearchQuery] = useState('');
  const [wholesalers, setWholesalers] = useState<WholesalerRecord[]>([]);
  const [statusCounts, setStatusCounts] = useState<Record<string, number>>({ pending: 0, on_hold: 0, verified: 0, banned: 0, rejected: 0, resubmission_required: 0 });
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      try {
        ...
        const { data } = await fetchWholesalers(selectedFilter, searchQuery, 1, 50);
        setWholesalers(data as WholesalerRecord[]);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        ...
        <h1 className="text-3xl font-light mb-8">Wholesaler submissions</h1>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Wholesaler</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                   <td colSpan={6} className="px-6 py-10 text-center text-gray-500">Loading wholesalers...</td>
                </tr>
              ) : wholesalers.length === 0 ? (
                <tr>
                   <td colSpan={6} className="px-6 py-10 text-center text-gray-500">No wholesalers found.</td>
                </tr>
                ...
                >
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium">{wholesaler.full_name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 text-gray-700">{wholesaler.business_name}</td>
                  <td className="px-6 py-5 text-gray-600">{wholesaler.city}, {wholesaler.state}</td>
                  <td className="px-6 py-5 text-gray-600 text-sm">{new Date(wholesaler.created_at).toLocaleDateString()}</td>
                  <td className="px-6 py-5">
                    {getStatusBadge(wholesaler.verification_status)}
                  </td>
                  <td className="px-6 py-5">
                    <Link 
                      to={`/review/${wholesaler.id}`}
                      className="text-black hover:text-gray-600 font-medium text-sm transition-colors"
                    >
                      {wholesaler.verification_status === 'verified' || wholesaler.verification_status === 'banned' ? 'View' : 'Review'}
                    </Link>
                  </td>
                </tr>
              ))}
